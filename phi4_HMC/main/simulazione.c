...
{
    int i, nVal = 14; /* numero di valori di naccu a D fissato */
    int naccu [] = {1,2,4,8,16,32,64,128,256,512,1024,2048,4096,8192};
    ...
    /* Iniziamo con le simulazioni a D, L, V, kappa fissati e variando naccu */
    /*for (i = 0; i < nVal; i++) {
        hmc_params.naccu = naccu[i];
        printf("naccu = %d\n",hmc_params.naccu);
        sprintf(nameFile,"naccu%d.txt",hmc_params.naccu);
        outFile = fopen(nameFile,"w");
        printMATLABCommentLine(outFile);
        fprintf(outFile,"%%SIMULAZIONE D = %d, L = %d, V = %d\n",D,L,V);
        fprintf(outFile,"%%NUMERO DI MISURE PER OGNI naccu: T = %d\n",hmc_params.ntraj/hmc_params.naccu);
        printMATLABCommentLine(outFile);
        fprintf(outFile,"%%naccu = %d\n",hmc_params.naccu);
        printMATLABCommentLine(outFile);
        fprintf(outFile,"\n");
        printMATLABCommentLine(outFile);
        fprintf(outFile,"\n");
    ...
    /* Ora, fissiamo la statistica da ottenere alla fine della simulazione e modifichiamo ntraj di conseguenza. Per esempio, qui usiamo ntraj = 100000 per naccu = 1, cioè T = 1000*/
    for (i = 0; i < nVal; i++) {
        hmc_params.naccu = naccu[i];
        hmc_params.ntraj = 1000*hmc_params.naccu;
        printf("naccu = %d, ntraj = %d\n",hmc_params.naccu,hmc_params.ntraj);
        sprintf(nameFile,"naccu%d_ntraj%d.txt",hmc_params.naccu,hmc_params.ntraj);
        outFile = fopen(nameFile,"w");
        printMATLABCommentLine(outFile);
        fprintf(outFile,"%%SIMULAZIONE D = %d, L = %d, V = %d\n",D,L,V);
        fprintf(outFile,"%%NUMERO DI MISURE PER OGNI naccu: T = %d\n",hmc_params.ntraj/hmc_params.naccu);
