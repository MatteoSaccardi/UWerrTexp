...
{
    double err_phi,err_mom,err_H;
    FILE* fileParametri = fopen("parametri.txt","w");
    /*
    int i,j,nBins;
    ...
    read_input(argv[1],fileParametri);
    fclose(fileParametri);
    ...
    /* check reversibility */
    check_reversibility(&act_params,&hmc_params,&err_phi,&err_mom,&err_H);
    return 0;
}
